...
本文件补充 `docs/P6-W7O-ACTIVE-SESSION-REVOKE-BASELINE.md` 在独立 Reviewer 审查后新增的安全修复。原 Baseline 继续作为当时工作包事实记录；Reviewer 修复尚缺授权运行环境的独立运行时验证，因此 `P6-W7-15` 不升级回 `DONE`。
...
`P6-W7-15` remains `VERIFYING` because the Reviewer execution policy does not run CI/CD, GitHub Actions, Runner or deployment verification. No runtime PASS is claimed here.

## External evidence still required

- Real AWKN final authorization remains P6-W8.
- Real revoked-Grant response timing and cross-service trace remain P6-W8.
- Runtime owner must execute the existing P6 verification suite against the corrective head and record evidence before W7-15 can return to `DONE`.

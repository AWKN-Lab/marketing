...
export function LearningWatchPanel({ workspaceId, workspaceName }: { workspaceId: string; workspaceName: string }) {
  const [watches, setWatches] = usePersistedState<LearningWatch[]>(LEARNING_WATCHES_KEY, []);
  const existing = useMemo(() => watches.find((watch) => watch.workspaceId === workspaceId), [watches, workspaceId]);
  const [topicInput, setTopicInput] = useState("");
  const [draftTopics, setDraftTopics] = useState<string[]>(existing?.topics ?? []);
  const [draftSources, setDraftSources] = useState<string[]>(existing?.sourceTypes ?? ["政策", "客户公开动态", "行业"]);
  ...
  function toggleSource(source: string) {
    setDraftSources((current) => current.includes(source) ? current.filter((item) => item !== source) : [...current, source]);
  }
  ...
  return <section className="panel stack-md">
    ...
    <div className="learning-contract"><span className="pulse"/><div><strong>{existing?.enabled ? "Watch Scope 已准备" : "等待启用"}</strong><p>{existing?.enabled ? "当前没有伪造学习结果。接入 LearningPort 后，由平台执行每日搜索与证据验证，再把真实 Signal 回写 Today。" : "启用后将保存当前关注范围；不在浏览器里自行实现搜索调度。"}</p></div></div>

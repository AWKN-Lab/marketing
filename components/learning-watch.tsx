...
export function LearningWatchPanel({ workspaceId, workspaceName }: { workspaceId: string; workspaceName: string }) {
  ...
  async function save(enabled: boolean) {
    ...
    const result = await callMarketingProduct({ operation: "learning.watch.upsert", workspaceId, payload: { watch: next } });
  ...
  async function runOnce() {
    ...
    const result = await callMarketingProduct<unknown>({
      operation: "learning.run",
      workspaceId,
      payload: { watch_id: existing.id, topics: existing.topics, source_types: existing.sourceTypes },
    });
    if (!result.ok) {
    ...
    if (!run) {
      setMessage("平台已响应，但缺少有效 run_id；本次结果未写入 Today。");
      setRunning(false);
      return;
    }
    persistRun(run);

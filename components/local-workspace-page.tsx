...
export function LocalWorkspacePage({ workspaceId }: { workspaceId: string }) {
  ...
  return <main className="page stack-lg">
    <header className="page-header"><div><p className="eyebrow">WORKSPACE / {workspace.type}</p><h1>{workspace.name}</h1><p className="muted">{workspace.goal}</p></div><NewTaskButton workspaceId={workspace.id} workspaceName={workspace.name} label="开始第一个任务" /></header>

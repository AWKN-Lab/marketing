...
export function LocalWorkspacePage({ workspaceId }: { workspaceId: string }) {
  const [workspaces, setWorkspaces, hydrated] = usePersistedState<LocalWorkspace[]>(LOCAL_WORKSPACES_KEY, []);
  ...
  return <main className="page stack-lg">
    ...
    <EntityReconcilePanel<LocalWorkspace> entityLabel="Workspace" entityKey={`workspace:${workspace.id}`} entityId={workspace.id} workspaceId={workspace.id} getOperation="workspace.get" updateOperation="workspace.update" localEntity={workspace} buildUpdatePayload={(entity, baseRevision) => ({ workspace: entity, base_revision: baseRevision })} onApplyPlatform={(remote) => setWorkspaces((current) => current.map((item) => item.id === workspace.id ? { ...remote, id: item.id } : item))} />

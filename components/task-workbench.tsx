  ...
  return (
    <main className="task-page">
      <header className="task-header">
        ...
        <div className="row gap-sm"><EvidenceDrawer scopeId={task.workspaceId}/><button className="button primary">继续运行</button></div>
      </header>
      <div className="task-grid">
        <TaskConversation task={task} />
        <ArtifactWorkspace taskId={task.id} workspaceId={task.workspaceId} title={task.artifact.title} aiDraft={task.artifact.aiDraft} initialFinal={task.artifact.userFinal} />
      </div>
    </main>
  );
}

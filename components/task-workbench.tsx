...
export function TaskWorkbench({ task }: { task: MarketingTask }) {
  ...
  return <main className="task-page">
    <header className="task-header"><div><p className="eyebrow">{task.workspaceName} / {task.type}{isLocal ? "" : " / DEMO"}</p><h1>{task.title}</h1><p className="muted">目标：{task.goal}</p></div><div className="row gap-sm"><EvidenceDrawer scopeId={task.workspaceId} includeDemo={!isLocal}/><button className="button primary">继续运行</button></div></header>

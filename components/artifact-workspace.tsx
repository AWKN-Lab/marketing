...
export function ArtifactWorkspace({ taskId, title, aiDraft, initialFinal }: { taskId: string; title: string; aiDraft: string; initialFinal: string }) {
  ...
  const diffSummary = useMemo(() => {
    ...
    return {
      removed: aiLines.filter((line) => !finalLines.includes(line)),
      added: finalLines.filter((line) => !aiLines.includes(line)),
    };
  }, [aiDraft, finalText]);
    ...
    <section className="artifact-panel">
      ...
      {mode === "edit" ? (
        ...
        <div className="diff-view">
          <div className="diff-column removed"><p className="eyebrow">REMOVED FROM AI</p>{diffSummary.removed.length ? diffSummary.removed.map((line, index) => <p key={`${index}-${line}`}>− {line}</p>) : <p className="muted">暂无删除</p>}</div>
          <div className="diff-column added"><p className="eyebrow">ADDED BY USER</p>{diffSummary.added.length ? diffSummary.added.map((line, index) => <p key={`${index}-${line}`}>+ {line}</p>) : <p className="muted">暂无新增</p>}</div>
        </div>
      )}

...
export function ArtifactWorkspace({ taskId, workspaceId, taskType, taskGoal, title, aiDraft, initialFinal }: { taskId: string; workspaceId: string; taskType: string; taskGoal: string; title: string; aiDraft: string; initialFinal: string }) {
  ...
  function createCandidate() {
    ...
    const candidate = createExperienceCandidate({ taskId, workspaceId, taskType, taskGoal, artifactTitle: title, feedback, outcome, outcomeNote, editCount: diffSummary.removed.length + diffSummary.added.length, fingerprint });
  ...
  return <section className="artifact-panel">
    ...
    {feedback && <OutcomeCapture value={outcome} note={outcomeNote} onChange={setOutcome} onNoteChange={setOutcomeNote} candidateCurrent={candidateCurrent} onCreateCandidate={createCandidate} />}
  </section>;
}

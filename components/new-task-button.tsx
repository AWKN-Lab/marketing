...
export function NewTaskButton({ workspaceId, workspaceName, label = "新任务" }: { workspaceId: string; workspaceName: string; label?: string }) {
  ...
  const matchedExperiences: AppliedExperience[] = candidates
    ...
    .slice(0, 3)
    .map((candidate) => ({ lesson: candidate.lesson, source: `${candidate.source} · 已审核` }));
  ...
  return <>
    ...
    {open && <div className="modal-layer" onClick={() => setOpen(false)}><section className="task-modal" onClick={(event) => event.stopPropagation()}>
      ...
      {matchedExperiences.length > 0 && <div className="matched-experience-preview"><p className="eyebrow">EXPERIENCE MATCH</p><strong>创建后将自动应用 {matchedExperiences.length} 条已确认经验</strong>{matchedExperiences.map((item) => <p key={item.source}>↗ {item.lesson}</p>)}</div>}

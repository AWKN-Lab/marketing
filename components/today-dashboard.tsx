...
export function TodayDashboard() {
  ...
  return <main className="page stack-xl">
    <header className="hero"><div><p className="eyebrow">TODAY</p><h1>今天值得推进什么</h1><p className="muted">真实 Workspace、任务、待审核经验与学习关注范围会回流到这里。</p></div><Link className="button primary" href="/workspaces">进入 Workspace</Link></header>
    ...
    <section className="panel stack-md demo-card"><div className="section-title"><div><p className="eyebrow">DEMO SIGNALS</p><h2>示例 Signal</h2></div><span className="pill demo-badge">未连接 LearningPort</span></div><p className="muted small">以下只用于界面验证；平台接入后仅展示真实来源和检索结果。</p>{signals.map((signal) => <article className="signal-row" key={signal.title}><span className="signal-time">{signal.time}</span><div><strong>{signal.title}</strong><p>{signal.impact}</p><small>{signal.source}</small></div></article>)}</section>
  </main>;
}

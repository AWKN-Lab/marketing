...
import { normalizeMarketingSession, type MarketingSession } from "@/lib/product-session";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) setError(cause instanceof Error ? cause.message : "SESSION_UNAVAILABLE");
      }
    };
    void load();

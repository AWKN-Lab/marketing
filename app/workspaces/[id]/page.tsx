...
export default async function WorkspacePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="page stack-lg">
      ...
      <section className="panel stack-md"><div className="section-title"><div><p className="eyebrow">TASKS</p><h2>最近任务</h2></div><NewTaskButton workspaceId={workspace.id} workspaceName={workspace.name}/></div>{workspace.tasks.map((task) => <Link href={`/tasks/${task.id}`} className="task-row" key={task.id}><div><strong>{task.title}</strong><p className="muted small">{task.type} · {task.status}</p></div><span>→</span></Link>)}</section>
    </main>
  );
}

...
export async function POST(request: Request) {
  ...
  if (!endpoint) {
    return NextResponse.json<MaterialUploadAck>({
      ok: false,
      error: { code: "PLATFORM_NOT_CONFIGURED", message: "AWKN 资料上传接口尚未配置；当前仅保留本地资料引用。", retryable: false },
    }, { status: 503 });
  }
  ...
  try {
    ...
    return NextResponse.json<MaterialUploadAck>({ ok: false, error: { code: "INVALID_MULTIPART", message: "资料上传必须使用 multipart/form-data。" } }, { status: 400 });
  }
  ...
  if (typeof workspaceId !== "string" || !workspaceId || typeof materialId !== "string" || !materialId || !(file instanceof File)) {
    return NextResponse.json<MaterialUploadAck>({ ok: false, error: { code: "INVALID_UPLOAD_REQUEST", message: "缺少 workspace_id / material_id / file。" } }, { status: 400 });
  }
  if (file.size <= 0) {
  ...
  outgoing.set("idempotency_key", `material.upload:${materialId}:${file.size}:${file.name}`);
  ...
  try {
    const upstream = await fetch(endpoint, {
      method: "POST",
      headers: process.env.AWKN_MARKETING_MATERIAL_UPLOAD_TOKEN
        ? { authorization: `Bearer ${process.env.AWKN_MARKETING_MATERIAL_UPLOAD_TOKEN}` }
        : {},
      body: outgoing,
    ...
    if (!payload) {
      return NextResponse.json<MaterialUploadAck>({ ok: false, error: { code: "INVALID_UPLOAD_UPSTREAM_RESPONSE", message: "AWKN 资料接口返回了非 JSON 响应。" } }, { status: 502 });
    }
    const normalized = normalizeMaterialUploadAck(payload, materialId);
    if (!normalized.ok && payload.ok) {
      return NextResponse.json<MaterialUploadAck>({ ok: false, error: normalized.error, trace_id: normalized.traceId }, { status: 502 });
    }

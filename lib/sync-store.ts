...
export type SyncRecord = { entityKey: string; operation: ProductOperation; state: SyncState; updatedAt: string; traceId?: string; error?: string; platformRevision?: number };
export const SYNC_RECORDS_KEY = "marketing:sync:records";
export const SYNC_EVENT = "awkn-marketing:sync";

export function syncRecordFromResponse(input: { entityKey: string; operation: ProductOperation; response: MarketingProductResponse; updatedAt?: string }): SyncRecord {
  ...
  return { entityKey: input.entityKey, operation: input.operation, state, updatedAt: input.updatedAt ?? new Date().toISOString(), traceId: input.response.trace_id, error: input.response.ok ? undefined : input.response.error?.message, platformRevision: typeof data?.revision === "number" ? data.revision : undefined };
}
...
function writeRecord(record: SyncRecord) {
  ...
  window.localStorage.setItem(SYNC_RECORDS_KEY, serializePersistedValue({ ...current, [record.entityKey]: record }));
...
export function readSyncRecord(entityKey: string) {
  ...
  return parsePersistedValue<Record<string, SyncRecord>>(window.localStorage.getItem(SYNC_RECORDS_KEY), {})[entityKey] ?? null;
}

export async function syncMarketingProduct<TPayload>(input: { entityKey: string; operation: ProductOperation; workspaceId?: string; taskId?: string; idempotencyKey?: string; expectedEntityId?: string; payload: TPayload }) {
